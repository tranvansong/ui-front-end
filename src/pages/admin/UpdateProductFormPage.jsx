...
import { getProductById, updateProduct } from "../../api/product/product";
...
const UpdateProductFormPage = () => {
  ...
  const [colors, setColors] = useState([
    {
      ...
      dropdownOpen: false,
      images: [],
      variants: [{id: "", size: "", stockQuantity: "", soldQuantity: "" }],
    },
  ]);
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        ...
        // Đảm bảo rằng dữ liệu `brands` và `categories` đã sẵn sàng
        if (brands.length && categories.length) {
          const matchedBrand = brands.find(
            (brand) =>
              brand.brand.toLowerCase() === productData.brand.toLowerCase()
          );
          ...
          setColors(
            productData.colors.map((color) => ({
              colorId: color.id,
              dropdownOpen: false,
              images: color.images.map((image) => ({ preview: image })),
    ...
    fetchProductDetails();
  }, [productId, brands, categories]); // Thêm `brands` và `categories` vào dependency
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.name) newErrors.name = "Tên sản phẩm không để trống";
      ...
      try {
        ...
        for (const color of colors) {
          for (const image of color.images) {
            if (image.file) {
              ...
              imageUrls.push(image.preview.image);
            }
          }
        }
        if (imageData.has("files")) {
          const urlImages = await uploadImage(imageData);
        ...
        const updatedColors = colors.map((color, index) => {
          const updatedImages = color.images.map(
            (_, imageIndex) =>
              imageUrls[index * color.images.length + imageIndex]
          );
        ...
        try {
          ...
          console.error(error);
          toast.error(error.data || "Có lỗi xảy ra");
        }
      } catch (error) {
        toast.error(error);
        console.error(error);
                      ...
                      >
                                ...
                                <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto">
                                  {colorsData.map((item) => (
                                    <div
                                      key={item.id}
                                      onClick={() =>
                                        handleColorChange(colorIndex, item.id)
                                      }
                        ...
                        <div className="mb-4">
                          ...
                          <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
                            {color.images.map((imageObj, imageIndex) => (
                              <div key={imageIndex} className="relative group">
                                <img
                                  src={imageObj.preview.image || imageObj.preview}
